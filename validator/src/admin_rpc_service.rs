...
impl AdminRpc for AdminRpcImpl {
    ...
    fn set_staked_nodes_overrides(
        ...
        let loaded_config = load_staked_nodes_overrides(&path_or_url)
            .map_err(|err| {
                error!("Failed to load staked nodes overrides from {}: {}", &path_or_url, err);
        ...
        info!("Staked nodes overrides was replaced with data from {}", path_or_url);
        Ok(())
    }
...
pub fn load_staked_nodes_overrides(
    ...
    debug!("Loading staked nodes overrides configuration from {}", path_or_url);
